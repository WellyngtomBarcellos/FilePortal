  ...
  return (
    <div className="inline-flex items-center gap-3 text-sm font-bold">
      ...
      {state === "connected" && ping !== null && (
        <span className="flex items-center gap-1.5 text-[10px] font-black text-white/30 border-l border-white/10 pl-3 ml-1 uppercase tracking-widest">
          <Activity className="h-3 w-3" />
          {ping}ms
        </span>
      )}
      {state === "connecting" && <Loader2 className="h-4 w-4 animate-spin text-white/40" />}
